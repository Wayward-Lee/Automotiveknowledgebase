...
Executive Services는 지상국으로부터 다양한 명령을 수신하여 시스템을 제어할 수 있다. 이 명령들은 애플리케이션 관리, 시스템 제어, 진단 정보 수집 등의 기능을 제공한다. ES 명령은 cFE Software Bus를 통해 ES 태스크로 전달되며, 각 명령은 고유한 명령 코드로 식별된다.

### 1.1 명령 분류

ES 명령은 기능에 따라 여러 범주로 분류된다. 기본 명령은 통신 확인과 카운터 리셋을 위한 것이고, 앱 제어 명령은 애플리케이션의 시작, 중지, 재시작을 담당한다. 시스템 제어 명령은 리셋이나 로그 출력 등의 시스템 레벨 동작을 수행하며, 성능 로그 명령은 타이밍 데이터 수집을 제어한다.
...
기본 명령은 ES의 가장 단순한 명령들로, 통신 확인과 카운터 초기화를 위해 사용된다. 이 명령들은 시스템 상태를 변경하지 않으며, 지상국과의 통신 링크가 정상적으로 동작하는지 확인하거나 테스트용으로 사용된다.

### 2.1 NOOP (No Operation)

NOOP 명령은 "No Operation"의 약자로, 아무런 동작도 수행하지 않는 명령이다. 의미 없어 보이지만 실제로 매우 유용하다. 지상국에서 우주선으로 명령을 전송하고 응답 이벤트를 수신함으로써 통신 링크가 정상적으로 동작하는지 확인할 수 있다. 또한 이 명령에 대한 응답에는 cFE 버전 정보가 포함되어 소프트웨어 버전을 확인하는 데도 사용된다.
...
애플리케이션 제어 명령은 ES의 가장 강력한 기능 중 하나이다. 지상에서 원격으로 애플리케이션을 시작, 중지, 재시작, 또는 재로드할 수 있다. 이를 통해 운영 중인 우주선의 소프트웨어를 동적으로 관리할 수 있다.

### 3.1 START_APP

START_APP 명령은 새로운 애플리케이션을 동적으로 로드하고 시작한다. 앱 파일 경로, 진입점 함수 이름, 앱 이름, 스택 크기, 우선순위, 예외 동작 등의 정보를 페이로드로 전달한다. 이 명령을 통해 초기 시작 스크립트에 없었던 앱도 나중에 추가할 수 있다.
...
ES는 지상으로 다양한 텔레메트리 패킷을 발송한다. 이 데이터를 통해 지상국은 시스템 상태, 리소스 사용량, 리셋 기록, 등록된 애플리케이션 정보 등을 모니터링할 수 있다. 텔레메트리는 관제실의 분석과 어노머리 탐지에 필수적인 정보를 제공한다.

### 7.1 Housekeeping 텔레메트리

Housekeeping 텔레메트리는 ES의 전반적인 상태를 요약하는 주기적 패킷이다. 명령 카운터, cFE/OSAL/PSP 버전 정보, 리셋 유형과 횟수, 등록된 앱 및 태스크 수, 메모리 사용량, Syslog 상태 등 다양한 정보가 포함된다. 이 패킷은 일반적으로 스케줄러에 의해 주기적으로 요청된다.
