...
Processor Reset(PR)은 Power-On Reset보다 덜 철저한 형태의 리셋으로, 소프트웨어 레벨에서 시스템을 재시작하면서도 중요한 데이터를 보존할 수 있다. CDS(Critical Data Store)에 저장된 데이터가 유지되므로, 애플리케이션들은 이전 세션의 상태를 복원할 수 있다. 이러한 특성 때문에 Processor Reset은 일시적인 오류 복구나 소프트웨어 유지보수에 주로 사용된다.

### 3.1 발생 원인

Processor Reset은 Power-On Reset과 달리 주로 소프트웨어적인 원인에 의해 발생한다. 가장 일반적인 경우는 지상국에서 리셋 명령을 전송하는 것이다. 이는 계획된 유지보수나 문제 해결을 위해 의도적으로 수행된다.

워치독 타이머 타임아웃도 Processor Reset의 중요한 원인이다. 워치독은 시스템이 정상적으로 동작하고 있는지 감시하는 하드웨어 메커니즘으로, 주기적으로 "킥(kick)"되어야 타임아웃이 방지된다. 소프트웨어가 교착 상태에 빠지거나 무한 루프에 갇히면 워치독을 킥하지 못하게 되고, 결국 타임아웃이 발생하여 프로세서가 리셋된다.

심각한 예외(Exception)가 발생했을 때 예외 처리 정책에 따라 Processor Reset이 트리거될 수 있다. 또한 애플리케이션이 API를 통해 명시적으로 리셋을 요청할 수도 있다.

Processor Reset의 주요 발생 원인은 다음과 같다:
...
Processor Reset이 발생하면 ES는 먼저 CDS의 유효성을 확인한다. CDS가 유효하면 저장된 데이터를 보존하고 리셋 카운터를 증가시킨다. 그러나 연속적인 Processor Reset 횟수가 미리 설정된 최대값을 초과하면, 이는 반복적인 리셋으로도 문제가 해결되지 않음을 의미하므로 시스템은 자동으로 Power-On Reset으로 전환된다. 이 안전 메커니즘은 시스템이 무한 리셋 루프에 빠지는 것을 방지한다.
...
ES는 리셋과 관련된 다양한 API 함수를 제공한다. 이 함수들을 통해 애플리케이션은 현재 리셋 유형을 확인하고, 필요한 경우 시스템 리셋을 요청할 수 있다. 리셋 유형에 따라 적절한 초기화 전략을 선택하는 것은 비행 소프트웨어 개발에서 매우 중요한 패턴이다.

### 4.1 리셋 정보 조회

`CFE_ES_GetResetType()` 함수는 현재 시스템이 어떤 유형의 리셋 후에 시작되었는지를 반환한다. 이 정보는 애플리케이션 초기화 시 중요한 결정을 내리는 데 사용된다. Power-On Reset 후에는 완전한 초기화가 필요하지만, Processor Reset 후에는 CDS에서 이전 상태를 복원할 수 있기 때문이다.
...
ES는 연속적인 Processor Reset 횟수를 추적하는 카운터를 유지한다. 이 카운터는 시스템의 안정성을 모니터링하고, 반복적인 리셋 문제를 감지하는 데 사용된다. 리셋 카운터가 최대값에 도달하면 시스템은 자동으로 Power-On Reset으로 전환되어 보다 철저한 초기화를 수행한다.

### 5.1 리셋 카운터 구조

ES는 리셋 관련 정보를 CDS에 보존되는 특별한 구조체에 저장한다. 이 구조체에는 마지막 리셋의 유형과 부유형, 연속 Processor Reset 횟수, 그리고 최대 허용 횟수 등이 포함된다. 이 정보는 Processor Reset을 통해서도 보존되므로, 연속적인 리셋 상황을 추적할 수 있다.
...
소프트웨어 실행 중에 심각한 예외(Exception)가 발생할 수 있다. 세그멘테이션 폴트(Segmentation Fault), 0 나누기 오류, 불법 명령어 실행 등이 대표적인 예외 상황이다. ES는 이러한 예외를 감지하고 애플리케이션별로 설정된 예외 동작 정책에 따라 적절히 대응한다.

### 6.1 예외 발생 시 리셋

예외가 발생하면 시스템은 해당 애플리케이션의 예외 동작 설정을 확인한다. 설정에 따라 두 가지 방식으로 대응할 수 있다. 첫 번째는 해당 앱만 재시작하는 것으로, 다른 앱들은 계속 정상적으로 동작한다. 두 번째는 전체 Processor Reset을 수행하는 것으로, 시스템 전체가 재시작된다.
